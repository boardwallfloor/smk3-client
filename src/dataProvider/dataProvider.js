...
const handleParamForFileUpload = async (params, resource) => {
  ...
  if (resource === 'reportsemester') {
    await Promise.all(
      Object.keys(files).map(async (key, index) => {
        if (files[key].file) {
          return (files[key].file.src = await changeBlobToBase64(
            files[key].file.src
          ))
        }
      })
  ...
  if (resource === 'reportyear') {
    await Promise.all(
      Object.keys(files).map(async (question, index) => {
        await Promise.all(
          Object.keys(files[question]).map(async (point1, i) => {
            if (files[question][point1].file) {
              // console.log(`files[${question}][${point1}]`)
              return (files[question][point1].file.src =
                await changeBlobToBase64(files[question][point1].file.src))
            }
            await Promise.all(
              Object.keys(files[question][point1]).map(async (point2, int) => {
                if (files[question][point1][point2].file) {
                  // console.log(`files[${question}][${point1}][${point2}]`)
                  return (files[question][point1][point2].file.src =
                    await changeBlobToBase64(
                      files[question][point1][point2].file.src
                    ))
                }
              })
            )
        ...
        if (question === 'question9' && files[question].file) {
          // console.log('test')
          return (files[question].file.src = await changeBlobToBase64(
            files[question].file.src
          ))
        }
      })
    )
...
export default {
  ...
  getOne: (resource, params) => {
    if (resource === 'profile') {
      ...
      // console.log(option)
      // console.log(userId)
      return httpClient(`${apiUrl}/user/${userId}`, option).then(
        ({ json }) => ({
      ...
      // console.log("Get One")
      return httpClient(`${apiUrl}/${resource}/${params.id}`, option).then(
        ({ json }) => ({
          data: { ...json, id: json._id }
        })
      )
  ...
  getMany: (resource, params) => {
    ...
    // console.log(url)
    return httpClient(url, option).then(({ json }) => ({
      data: json.map((resource) => ({ ...resource, id: resource._id }))
    }))
  },
  ...
  create: async (resource, params) => {
    if (resource === 'reportyear' || resource === 'reportsemester') {
      // console.log(params)
      await handleParamForFileUpload(params, resource).then((results) => {
