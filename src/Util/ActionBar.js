...
import { EditButton, TopToolbar, CreateButton, sanitizeListRestProps, usePermissions } from 'react-admin'
...
const FilterPrompt = (props) => {
  ...
  const massExport = async () => {
    ...
    const fileName = `${
            props.resource
        } - ${currentDate.getFullYear()}.${currentDate.getMonth()}.${currentDate.getDate()}.xlsx`
  ...
  const constructQuery = (permissions) => {
    ...
    return `${process.env.REACT_APP_API_LINK}/${
            props.resource
        }/exportall?filter=${JSON.stringify(filter)}`
  }

  useEffect(() => {
    const fetchInstitution = async () => {
      const institutionData = await fetch(
                `${process.env.REACT_APP_API_LINK}/institution?select=_id+name`
      )
    ...
    const fetchUser = async () => {
      ...
      console.log(
                `${process.env.REACT_APP_API_LINK}/user?select=_id+username&filter=${limitInstitution}`
      )
...
export const ExportButtonShow = ({ basePath, data, resource }) => {
  ...
  const exportData = async () => {
    const file = await fetch(
            `${process.env.REACT_APP_API_LINK}/${resource}/export/${data.id}`,
            {
              headers: {
                Authorization: `Bearer ${localStorage.getItem('jwt')}`
              }
